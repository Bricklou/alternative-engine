...
namespace AltE::Rendering {
  ...
  DescriptorSetLayout::~DescriptorSetLayout() {
    vkDestroyDescriptorSetLayout(_device.get_device(), _descriptor_set_layout,
                                 nullptr);
  }
  ...
  DescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType,
                                       uint32_t count) {
    _pool_sizes.push_back({descriptorType, count});
    return *this;
  }
  ...
  std::unique_ptr<DescriptorPool> DescriptorPool::Builder::build() const {
    return std::make_unique<DescriptorPool>(_device, _max_sets, _pool_flags,
                                            _pool_sizes);
  }
  ...
  DescriptorPool::DescriptorPool(
    ...
    if (vkCreateDescriptorPool(_device.get_device(), &descriptorPoolInfo,
                               nullptr, &_descriptor_pool) != VK_SUCCESS) {
  ...
  bool DescriptorPool::allocateDescriptor(
    ...
    if (vkAllocateDescriptorSets(_device.get_device(), &allocInfo,
  ...
  void DescriptorPool::freeDescriptors(
    ...
    vkFreeDescriptorSets(_device.get_device(), _descriptor_pool,
                         static_cast<uint32_t>(descriptors.size()),
                         descriptors.data());
  }

  void DescriptorPool::resetPool() {
    vkResetDescriptorPool(_device.get_device(), _descriptor_pool, 0);
  }
  ...
  DescriptorWriter::DescriptorWriter(DescriptorSetLayout &setLayout,
                                     DescriptorPool &pool)
      : _set_layout{setLayout}, _pool{pool} {}

  DescriptorWriter &
  DescriptorWriter::writeBuffer(uint32_t binding,
                                VkDescriptorBufferInfo *bufferInfo) {
    assert(_set_layout._bindings.count(binding) == 1 &&
           "Layout does not contain specified binding");

    auto &bindingDescription = _set_layout._bindings[binding];
  ...
  bool DescriptorWriter::build(VkDescriptorSet &set) {
    bool success =
        _pool.allocateDescriptor(_set_layout.get_descriptor_set_layout(), set);
    if (!success) {
      return false;
    }
    overwrite(set);
  ...
  void DescriptorWriter::overwrite(VkDescriptorSet &set) {
    ...
    vkUpdateDescriptorSets(_pool._device.get_device(), _writes.size(),
